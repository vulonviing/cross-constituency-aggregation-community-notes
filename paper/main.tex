...
canonical prompts, published in full in the repository. Rubric-based LLM-as-a-judge measurement has broader
...
displays no note. Quotations are abridged with [\ldots] and each note's linked
sources are named in brackets rather than printed; the full text and URLs are
in the repository. ``Global'' is overall approval, ``CCA score'' is the
geometric mean of the two constituency-specific approval rates, and ``Gemma''
is the Stage 2 rescue-worthiness score.}
...
note and the different, higher-scoring CCA selection. Quotations are abridged
and sources named as in Table~\ref{tab:hidden-rescue-cases}. ``Global'' is
overall approval, ``CCA
