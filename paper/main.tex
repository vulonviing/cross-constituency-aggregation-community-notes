...
\citep{goyal2026qsmf,nudo2026hyperactive}, and participation is steeply unequal
(Section~\ref{sec:the_gap}), so a formally open crowd can still turn on a small,
active core. More basically, the model keeps group support inside a fitted
...
The ladder also shows why a stability score cannot be read as a certificate. Eight of the 19 variants scored bootstrap ARI between 0.976 and 1.000 for a partition that merely fences off roughly 60 power-voters, at 250k splitting 249{,}933 against 67 (Table~\ref{tab:scaleup}). Such a partition is perfectly reproducible and analytically worthless: the most reproducible object in the rating record is the power-voter clique, not the division between camps.
...
around it. Legitimacy in this paper means something thin and procedural:
whether the groups affected by a decision have reason to accept the way it was
made. States face a heavier version of the same problem, which is what makes
their solutions a conservative place to look.
...
has to act as a hard veto in practice, and that scarcity is itself evidence
the requirement is doing its job quietly: almost every note that reaches a
bridge score reaches it having been heard from both sides, and the picks below
the CCA threshold fail on the merits of the ratings they received, not on a
technicality.
