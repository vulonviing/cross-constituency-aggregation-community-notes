...
if a note gathers enough support it is shown under the post. Rather than
professional fact-checkers or automated classifiers, a crowd decides which
context becomes visible \citep{wojcik2022birdwatch,xcommunitynotes2026ranking}.
...
passing a law, but both settings face the same aggregation problem under
disagreement. It yields four criteria for the display decision:
...
Visibility is not decided by majority vote but by a matrix factorization designed to find consensus across polarized groups, which the platform calls ``bridging'' \cite{wojcik2022birdwatch}. It decomposes the predicted rating $\hat{r}_{un}$ that rater $u$ gives note $n$ as:
...
The terms $i_u$ and $f_u \cdot f_n$ do not measure ``bad faith''; they act as normalization tools. But by relying on them, the core action of Community Notes shifts from evaluating the \textit{content} of a note to estimating the \textit{intent} of the user. Simulation studies of the open-source algorithm make this concrete: a coordinated minority of just 5--20\% of biased raters can suppress genuinely helpful notes outright \citep{truong2025vulnerable}. Observers have questioned the system's narrow reliance on cross-group consensus \citep{jude2025narrow}; our concern is more specific. To isolate note quality ($i_n$), the algorithm treats rater tendencies and viewpoint compatibility as structural noise (Figure~\ref{fig:rater-dominance-schematic}), and in doing so profiles users and reads their intentions: is this user voting on objective truth, or cheering for their side? The result is an implicit hierarchy of raters, on the assumption that weighting ``high-quality,'' unbiased evaluators is the most effective way to govern visibility.
...
\begin{figure*}[t]
  ...
  the fitted latent space. No explicit vote weight is assigned; their longer
  histories simply contribute more observations to the estimates of $i_u$ and
  $f_u$, which the pipeline uses before isolating $i_n$. Schematic, not to
  scale.}
...
\bottomrule
\end{tabular}}
\caption{Clustering stability across scale. The initial columns report the partition selected on the raw co-rating graph before reassignment, with $k$ chosen by bootstrap stability and ARI the mean bootstrap ARI at that $k$; above 60k this partition still holds the hyperactive raters apart. The Method B columns report the final two-camp partition after vote-profile reassignment, on notes with at least ten raters per camp. Pearson correlates the camps' note-level approval rates; Discrim.\ is the share of notes whose between-camp gap exceeds 0.3. The last column gives the separated sub-population and its median vote count relative to the main camps.}
\label{tab:scaleup}
\begin{flushleft}
\footnotesize
...
source of non-compensatoriness. A key vote of the Northern Ireland Assembly
requires unionist and nationalist support at once, and support from one side
cannot be traded for a victory on some other bill. Acceptability must hold
within that decision, with neither side dispensable, exactly the structure a
geometric-mean soft veto encodes rather than a weighted average. Power-sharing
of this kind is not costless, and Section~\ref{sec:limitations} returns to the
limits of the borrowing.
...
logic formally. A legitimate collective decision should remain responsive to the
...
proportional representation makes the point axiomatically: sufficiently large
cohesive coalitions must not be washed out by majority aggregation
...
that decides under structured disagreement, each answering to one component of
the CCA rule. Behavioral rater clusters are not nations in the constitutional
sense and we do not claim they hold such groups' normative standing; the
criteria are \emph{weakened} aggregation requirements adapted to platform
governance. Table~\ref{tab:principles-map} collects each with its source and the
...
independent of relative size or rating activity. \emph{(The geometric mean is
symmetric in its arguments and takes approval rates, not raw counts.)}
...
that abstention from ever standing in for support, so both constituencies must
contribute at least three raters before the note is scored at all. Three
ratings do not measure a constituency's approval precisely; they establish that
it was there. Community Notes applies minimums of the same shape in its own
scoring \citep{xcommunitynotes2026ranking}, and set at three the requirement
rarely bites (Section~\ref{sec:results}). For a note that clears this coverage
bar, the cross-constituency score is the geometric mean
\begin{equation}
...
Read against these criteria, the production model's gap can be stated
precisely. Eq.~\eqref{eq:cn-model} contains a counterpart to P2, since the
bridging term credits helpfulness ($i_n$) only for approval the viewpoint match
cannot explain; but the constituency form of P2, that acceptability hold
separately within each group, cannot be stated at all, because groups are not
model objects. P1 has no counterpart either: pooled estimation has no notion of
``a group being absent.'' P3 is likewise absent, since a larger or more active
bloc simply contributes more terms to the loss, and the polarization safeguard
conflicts directly with P2, discounting notes on \emph{polarizing issues} rather
than notes with \emph{one-sided support}. The reliability-weighted extension
...
disagreement itself. We require three things of a partition before it counts.
It must \emph{persist}: under resampling the algorithm keeps recovering two
large camps beside a small hyperactive cluster at every non-degenerate scale,
and the two-camp partition reappears across sample sizes with the between-camp
correlation and discriminating share barely moving (Table~\ref{tab:scaleup}).
Persistence is nowhere near sufficient, as the ladder's own failures show: the
variants that merely fence off a power-voter clique score highest of all on
stability. Each side must therefore be \emph{large} enough that
...
power-voters are not a third, since under a symmetric rule their veto would
weigh as much as that of a camp a thousand times larger. And the two must
\emph{discriminate}, reaching different verdicts on a large share of notes;
about 81\% do. A partition meeting all three is not an arbitrary artifact but
the lines along which acceptance and rejection actually run. The standing is
conditional: when rating behavior no longer shows a stable cluster structure,
CCA's precondition fails and pooled estimation regains its warrant.
...
setup, with its thresholds set out in Appendix~\ref{app:pipeline}.
Figure~\ref{fig:dataset-construction} gives the filter chain: from 1.7M
...
ratings per note, secure a genuine multi-note choice set and minimum observed
coverage (Appendix~\ref{app:filters}); the 48-hour rating window keeps ratings
tied to the note's active review period. The slice spans 44,722 distinct tweets,
the unit from which the Representative strategy selects one note. This dense
core is deliberate: co-rating similarity is informative only when users rate
enough of the same notes to be compared.
...
\begin{figure}[h]
  ...
  tweets carrying at least three English candidate notes and to notes drawing at
  least three eligible ratings within 48 hours, yielding 510k; from these a
  100k-note dense slice (about the top 20\% by rating volume) and its 200k active
  raters.}
...
recurring clique of power-voters. Selecting $k=3$ makes both visible: two large
camps and a third cluster of 64 highly active raters, too small to be granted
...
attempt statuses are recorded, so any judgment can be regenerated independently
...
in both directions. Setting those 82 aside, the 13,655 CCA-qualified notes
Community Notes does not display form the rescue pool that the rest of this
section evaluates (Table~\ref{tab:rep-cn-overlap},
Figure~\ref{fig:rescue-panels}). Coverage is rarely why the remaining picks fall
short: only 71 never receive three ratings from both constituencies, and across
the full 100k-note slice only 185 notes of any kind go unscored for lack of
coverage. The rest are scored and simply fall short, at a median bridge score of
0.291. Most of what CCA declines, it declines on the merits of the ratings it
has.
...
unsourced claims, and 142 as hostile in tone, with one left unresolved. Stage
1.5 rechecks the opinion-labeled notes and recovers 280, giving an expanded
Stage 2 population of 10,376 scored for rescue-worthiness. Of these, 8,558 clear the 50-point threshold, 62.7\% of
...
from the 8,339 Gemma-validated selections on tweets where Community Notes
displays no note, and three from the 219 where the tweet already has a different
displayed note. They illustrate the rule's behavior rather than estimate the
average quality of either population.
...
Section~\ref{sec:the_gap}, which holds at every scale from 60k to 240k raters
...
several substantive limits accompany the design.
...
two judgments, but we tested no retrieval-enabled setup, no other or larger
model, and no sensitivity sweep over the 0.5 bridge and 50-point thresholds; the
run uses one temperature and has no human-rated comparison sample. Content validation is not immune to the
